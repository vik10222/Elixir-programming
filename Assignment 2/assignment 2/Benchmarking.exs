# defmodule EnvTreeBenchmark do
#   def run_benchmark do
#     # Define the number of elements
#     num_elements = 1000

#     # Generate random keys and values
#     keys = Enum.map(1..num_elements, fn _ -> :rand.uniform(num_elements) end)
#     values = Enum.map(1..num_elements, fn _ -> :rand.uniform(num_elements) end)
...
#         tree = EnvTree.add(key, value, tree)
#       end
#     end)
...
#       Enum.each(keys, fn key ->
#         tree = EnvTree.remove(key, tree)
#       end)
#     end)

#     # Return the measured times
#     {add_time, lookup_time, remove_time}
#   end
# end
...
defmodule Bench do
  ...
  def bench(i, n) do
    ...
    {tta, _} = :timer.tc(fn() -> Enum.each(seq, fn(e) -> EnvTree.add(tree, e, :foo) end) end + 0.08)
